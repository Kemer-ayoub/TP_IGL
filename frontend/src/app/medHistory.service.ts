...
export class MedHistoryService {
  ...
  private readonly ANT_URL = `${this.API_URL}antecedants/`;
  ...
  getAntecedants(dpi: any): Observable<any> { 
    const makeRequest = (token: string) => {
      const headers = new HttpHeaders({
        Authorization: `Bearer ${token}`
      });
      return this.http.get(this.ANT_URL, { headers, params: {dpi: dpi} });      
    };
  ...
  createAntecedants(antec: any): Observable<any> { 
    const makeRequest = (token: string) => {
      ...
      return this.http.post(this.ANT_URL, antec, { headers });      
    };
    const tokenData = JSON.parse(localStorage.getItem(this.JWT_TOKEN) || '{}');
    const accessToken = tokenData.access;
