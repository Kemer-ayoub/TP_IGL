...
export class ConsultationService {
  ...
  private readonly CONSUL_URL = `${this.API_URL}consultation/`;
  ...
  addConsultation(Consultation: any): Observable<any> {
    const makeRequest = (token: string) => {
      ...
      return this.http.post(this.CONSUL_URL, Consultation, { headers });
    };
  ...
  getConsultation(id: any): Observable<any> { 
    const makeRequest = (token: string) => {
      ...
      return this.http.get(this.CONSUL_URL, { headers, params: {id: id} });      
    };
    const tokenData = JSON.parse(localStorage.getItem(this.JWT_TOKEN) || '{}');
    const accessToken = tokenData.access;
  ...
  getAllConsultation(dpi: any): Observable<any> { 
    const makeRequest = (token: string) => {
      const headers = new HttpHeaders({
        Authorization: `Bearer ${token}`
      });
      return this.http.get(this.CONSUL_URL, { headers, params: {dpi: dpi} });      
    };
    const tokenData = JSON.parse(localStorage.getItem(this.JWT_TOKEN) || '{}');
    const accessToken = tokenData.access;
